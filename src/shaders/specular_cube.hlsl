...
vec3 ImportanceSampleGGX(float2 Xi, vec3 N, float Roughness)
{
    float a = Roughness*Roughness;
...
vec4 PixelMain(pixel_input In) : SV_TARGET
{
    ...
    for(uint i = 0u; i < SAMPLE_COUNT; ++i)
    {
        ...
        if(NdotL > 0.0)
        {
            ...
            float resolution = CubeWidth; // resolution of source cubemap (per face)
            float saTexel  = 4.0 * PI / (6.0 * resolution * resolution);
            float saSample = 1.0 / (float(SAMPLE_COUNT) * pdf + 0.0001);

            float mipLevel = Roughness == 0.0 ? 0.0 : 0.5 * log2(saSample / saTexel); 
            
            PrefilteredColor += environmentMap.SampleLevel(SampleType, L, mipLevel).rgb * NdotL;
            TotalWeight      += NdotL;
        }
    }
